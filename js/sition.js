...
function Sition(dom, nowAction, callBack, debug) {
    ...
    obj.debugInfo = function(str) {
        if (obj.debug == true) console.log(str);
    }
    ...
    obj.Listener = function(evt) {
        ...
        try {
            ...
            if (yValue < obj.sValue * 0.5 && obj.checkX(x)) {
                action = 'up';
            } else if (yValue > obj.sValue && obj.checkX(x)) {
                action = 'down';
            }			
    ...
    obj.End = function(evt) {
        ...
        if (obj.action + 'Ing' != obj.nowAction + 'Ing') obj.diffeValue = 0;
    ...
    obj.Init = function() {
        ...
        dom.addEventListener('touchstart', obj.Start, {
            passive: false
        });
        dom.addEventListener('touchmove', obj.Listener, {
            passive: false
        });
        dom.addEventListener('touchend', obj.End, {
            passive: false
        });
    }
    obj.Init();
